...
export default function Dashboard() {
  ...
  const getData = async () => {
    ...
    //collectionlardan usrin postlarinini getir
    const q = query(collectionRef, where("user", "==", user.uid));
    //uid userin id si, user ile user uid eslesiyorsa
  ...
  return (
    <container className="flex  flex-col justify-center items-center w-full h-fit p-4">
      <div
        className="w-2/3 max-w-lg  p-2 flex flex-col items-center justify-center text-center gap-2 "
      >
              ...
              <Message {...post} key={post.id}>
                <div className="flex  flex-row items-center justify-center gap-3  text-center mt-1 ">
                  <button
                    ...
                    className="text-pink-700 flex items-center justify-center  py-2 text-sm r"
                  >
                  ...
                  {/* //tiklarsan postu alip pathname: "/post adresine gider  */}
                </div>
              </Message>
            );
          })}
